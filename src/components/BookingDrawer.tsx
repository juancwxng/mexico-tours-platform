...
export default function BookingDrawer({ tourTitle, tourPrice }: BookingDrawerProps) {
  ...
  return (
    <>
      ...
      {/*
        Single fixed container holding BOTH the sliding panel and the bottom bar.
        Uses flex-col-reverse so the bar stays at the bottom and the panel
        grows upward. max-height animation avoids iOS absolute-positioning bugs.
      */}
      <div className="lg:hidden fixed bottom-0 inset-x-0 z-50 flex flex-col-reverse pb-[env(safe-area-inset-bottom)]">

        {/* Sticky bottom bar — always visible */}
        <div className="bg-white border-t border-gray-100 px-4 py-3 flex items-center justify-between gap-4 shadow-lg flex-shrink-0">
          <div>
          ...
          <button
            ...
            className="flex items-center gap-2 bg-central-blue text-white font-bold text-sm uppercase tracking-wide px-6 h-12 rounded-xl hover:bg-central-blue/90 transition-colors flex-shrink-0"
          >
            <ChevronUp
